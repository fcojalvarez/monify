...
<template>
  ...
  <div v-else class="space-y-4">
    <BaseSpinner v-if="family.loading && !family.items.length" />

    <EmptyState v-else-if="!family.items.length" icon="solar:users-group-rounded-linear" :title="t('family.empty')" />
    ...
    <ul v-else class="space-y-1">
      <li v-for="member in family.items" :key="member.id"
        ...
        <button class="flex h-8 w-8 items-center justify-center rounded-full text-content-muted hover:bg-line"
          :aria-label="t('family.editAria')" @click="mode = { view: 'form', member }">
        ...
        <button v-if="!member.is_self"
          ...
          :aria-label="t('family.deleteAria')" @click="onDelete(member)">
  ...
  <BaseDialog v-model="showDeleteDialog" variant="danger" :title="t('family.deleteTitle')"
    :confirm-text="t('family.deleteAria')" show-cancel @confirm="confirmDelete">
    <p class="text-content">
      {{ t('family.deleteConfirmPrefix') }}
      <strong>{{ memberToDelete?.name }}</strong>?
    </p>

    <p class="mt-2 text-sm text-content-subtle">
      {{ t('family.deleteAssociatedWarning') }}
    </p>
