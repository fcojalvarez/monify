...
const mountWith = (component: any, initialState: Record<string, any>) =>
  mount(component, {
    global: {
...
describe('vistas de efectivo y ahorros', () => {
  ...
  it('muestra las carteras de efectivo y abre el diálogo de ingreso para una persona', async () => {
    ...
    await wrapper
      .findAll('button')
      .find((button) => button.text().includes('Añadir'))!
      .trigger('click')
    expect(wrapper.find('.dialog[data-title="Añadir efectivo"]').exists()).toBe(true)
  })

  it('permite abrir el menú de gestión cuando no existen metas', async () => {
    ...
    const manageButton = wrapper
      .findAll('button')
      .find((button) => button.text().toLowerCase().includes('gestionar'))
